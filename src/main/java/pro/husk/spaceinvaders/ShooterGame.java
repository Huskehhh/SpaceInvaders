...
public class ShooterGame extends PApplet {
    ...
    private void tickEnemies() {
        ...
        ImmutableList<Enemy> enemyImmutableList = ImmutableList.copyOf(Enemy.getEnemies());
        enemyImmutableList.forEach(Enemy::tick);
    }
    ...
    private void tickBullets() {
        ImmutableList<Bullet> bulletImmutableList = ImmutableList.copyOf(Bullet.getBullets());

        if (bulletImmutableList.size() != 0) {
            bulletImmutableList.forEach(bullet -> {
                bullet.tick();
